                            ...
                            <div className="floating-input relative" data-placeholder={field.label}>
                              {field.type === "select" ? (
                                <div className="relative">
                                  <button 
                                    type="button" 
                                    onClick={() => setFormValues(p => ({ ...p, [`${field.name}Open`]: !p[`${field.name}Open`], [`${field.name}Search`]: "" }))} 
                                  ...
                                  >
                                    <span className="truncate">{formValues[field.name] || `Select ${field.label}`}</span>
                                    <ChevronDown size={16} />
                                  </button>
                                  {formValues[`${field.name}Open`] && (
                                    <div className="absolute z-50 mt-1 max-h-48 w-full rounded border border-gray-100 bg-white shadow overflow-auto">
                                      <input 
                                        ...
                                        onChange={e => setFormValues(p => ({ ...p, [`${field.name}Search`]: e.target.value }))} 
                                      ...
                                      {field.options?.filter(opt => opt.label.toLowerCase().includes((formValues[`${field.name}Search`] || "").toLowerCase())).map(opt => (
                                        <div 
                                          ...
                                          onClick={() => { 
                                            ...
                                            setFormValues(p => ({ ...p, [`${field.name}Open`]: false })); 
                                          }} 
                                      ...
                                      {field.options?.filter(opt => opt.label.toLowerCase().includes((formValues[`${field.name}Search`] || "").toLowerCase())).map(opt => (
                                        <label key={opt.value} className="flex items-center px-4 py-2 hover:bg-gray-100 cursor-pointer">
                                          <input 
                                            type="checkbox" 
                                            className="mr-2" 
                                            checked={Array.isArray(formValues[field.name]) && formValues[field.name].includes(opt.value)} 
                                            onChange={e => {
                                ...
                                <>
                                  <input 
                                    ...
                                    className={`peer px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2  ${field.isDuration ? 'w-24 text-center' : 'w-full'}`}
                                    placeholder=" " 
                                    min={field.min} 
                                    max={field.max} 
                                    step={field.step} 
                                  ...
                                  {field.normalRange && (
                                    <div className="flex items-center gap-1 mt-1 text-xs text-yellow-600">
                                      <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-4 h-4 text-yellow-500">
                                        <path strokeLinecap="round" strokeLinejoin="round" d="M12 9v2.25m0 2.25h.008v.008H12v-.008zm.75-8.25a.75.75 0 11-1.5 0 .75.75 0 011.5 0zM21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                                      </svg>
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                    <div className="space-y-4">
                      <div>
                        ...
                        <input type="file" accept="image/*" onChange={handleSignatureUpload} className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2" />
                      </div>
                      {doctorSignature && (
                    ...
                    <div className="space-y-4">
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <button onClick={handleClearSignature} className="px-4 py-2 bg-red-600 text-white rounded-md hover:bg-red-700 flex items-center gap-1">
                  ...
                  <div className="flex items-center rounded-xl bg-gradient-to-r from-[#01B07A] to-[#1A223F] p-5">
                    ...
                    <div>
                      <p className="text-2xl font-semibold text-white">
                        {viewFields.find(f => f.titleKey) ? data[viewFields.find(f => f.titleKey).key] || "-" : "-"}
                      </p>
                      <p className="mt-1 text-sm font-medium text-blue-100 tracking-wide">
                        {viewFields.find(f => f.subtitleKey) ? data[viewFields.find(f => f.subtitleKey).key] || "-" : "-"}
                      </p>
                    </div>
                  </div>
