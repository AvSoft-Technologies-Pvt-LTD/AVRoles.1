...
const ReusableModal = ({
  ...
  onChange, onFieldsUpdate, shouldValidate = false, 
}) => {
  ...
  // Update fields based on form values for conditional logic
  useEffect(() => {
      ...
      <motion.div
        ...
        {(mode === "add" || mode === "edit" || mode === "viewProfile") && (
          <div className="sticky top-0 z-20 bg-gradient-to-r from-[#01B07A] to-[#004f3d] rounded-t-xl px-6 py-4 flex items-center justify-between">
            ...
            <button onClick={onClose} className="flex h-8 w-8 items-center justify-center rounded-full border border-white text-white hover:bg-gradient-to-br from-[#E6FBF5] to-[#C1F1E8] hover:text-[#01B07A] transition">
              <X size={18} />
            </button>
          </div>
        )}
        <div className="flex flex-col max-h-[90vh] overflow-visible bg-gradient-to-br from-[#E6FBF5] to-[#C1F1E8]">
          <div className="flex-1 p-4 relative z-0" style={{ overflow: 'visible' }}>
                        ...
                        <div key={field.name} className={`col-span-1 ${getColSpanClass(field.colSpan)} flex flex-col`}>
                          {field.type === "checkbox" ? (
                            <label className="inline-flex items-center gap-2 text-sm mt-1">
                              <input type="checkbox" name={field.name} checked={!!formValues[field.name]} onChange={e => handleChange(field.name, e.target.checked)} className="h-4 w-4 text-blue-600" />
                            ...
                            <div className="floating-input relative" data-placeholder={field.label}>
                              {field.type === "select" || field.type === "multiselect" ? (
                                <div className="relative">
                                  <button type="button" onClick={() => setFormValues(p => ({ ...p, [`${field.name}Open`]: !p[`${field.name}Open`], [`${field.name}Search`]: "" }))} className="peer w-full px-3 py-2 border border-gray-300 rounded-md text-left bg-white focus:outline-none focus:ring-2 flex justify-between items-center">
                                    <span className="truncate">{field.type === "multiselect" ? (Array.isArray(formValues[field.name]) && formValues[field.name].length ? `${formValues[field.name].length} selected` : `Select ${field.label}`) : (formValues[field.name] || `Select ${field.label}`)}</span>
                                    <ChevronDown size={16} />
                                  </button>
                                ...
                                <textarea name={field.name} value={formValues[field.name] || ""} onChange={e => handleChange(field.name, e.target.value)} rows={3} className="peer w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2" placeholder=" " />
                              )  :
 field.type === "date" ? (
                                <input type="date" name={field.name} value={formValues[field.name] || ""} onChange={e => handleChange(field.name, e.target.value)} className="peer w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2" />
                              ) : field.type === "file" ? (
                                <div className="relative flex items-center">
                                  ...
                                  {!field.multiple && formValues[field.name] && (
                                    <button type="button" onClick={() => { setPreviewFile(formValues[field.name]); setShowPreviewModal(true); }} className="absolute right-2 text-blue-600"><Eye size={18} /></button>
                                  )}
                                </div>
                              ) : (
                                <>
                                  <input type={field.type || "text"} name={field.name} value={formValues[field.name] || ""} onChange={e => handleChange(field.name, e.target.value)} readOnly={field.readonly} className={`peer px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 ${field.isDuration ? 'w-24 text-center' : 'w-full'}`} placeholder=" " min={field.min} max={field.max} step={field.step} />
                                  {field.unit && <span className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 text-xs pointer-events-none">{field.unit}</span>}
                                  {field.hasInlineCheckbox && (
                                    <div className="mt-3 flex items-center gap-3  rounded-lg ">
                                      <div className="flex items-center">
                                        <input 
                                          type="checkbox" 
                                          id={field.inlineCheckbox.name}
                                          name={field.inlineCheckbox.name} 
                                          checked={!!formValues[field.inlineCheckbox.name]} 
                                          onChange={e => handleChange(field.inlineCheckbox.name, e.target.checked)} 
                                          className="sr-only" 
                                        />
                                        <label 
                                          htmlFor={field.inlineCheckbox.name}
                                          className={`relative flex items-center justify-center w-5 h-5 rounded border-2 cursor-pointer transition-all duration-200 ${
                                            formValues[field.inlineCheckbox.name] 
                                      ...
                                      <div className="flex-1">
                                        ...
                                        >
                                          {field.inlineCheckbox.label}
                                        </label>
                                      </div>
                                    </div>
                                  )}
                                  {field.normalRange && <div className="flex items-center gap-1 mt-1 text-xs text-yellow-600"><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-4 h-4 text-yellow-500"><path strokeLinecap="round" strokeLinejoin="round" d="M12 9v2.25m0 2.25h.008v.008H12v-.008zm.75-8.25a.75.75 0 11-1.5 0 .75.75 0 011.5 0zM21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg><span>Normal Range: {field.normalRange}</span></div>}
                                </>
                              )}
                            </div>
                          )}
                ...
                <div className="bg-white rounded-xl shadow-lg border border-gray-200 p-8 animate-fadeIn">
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                    <div className="space-y-4">
                      <div><label className="block text-sm font-medium text-gray-700 mb-2">Upload Signature:</label><input type="file" accept="image/*" onChange={handleSignatureUpload} className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2" /></div>
                      {doctorSignature && (
                    ...
                    <div className="space-y-4">
                      <label className="block text-sm font-medium text-gray-700">Or Draw Signature:</label>
                      ...
                      <div className="flex items-center gap-3">
                        <button className="px-4 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700 flex items-center gap-1"><Save className="w-4 h-4" />Save</button>
                        <button onClick={handleClearSignature} className="px-4 py-2 bg-red-600 text-white rounded-md hover:bg-red-700 flex items-center gap-1"><X className="w-4 h-4" />Clear</button>
                      </div>
                    </div>
                  </div>
                </div>
              ...
              {mode === "viewProfile" && (
                <>
                  <div className="flex items-center rounded-xl bg-gradient-to-r from-[#01B07A] to-[#1A223F] p-5">
                    ...
                    <div>
                      <p className="text-2xl font-semibold text-white">{viewFields.find(f => f.titleKey) ? data[viewFields.find(f => f.titleKey).key] || "-" : "-"}</p>
                  ...
                  <div className="rounded-xl p-6 bg-white">
                    <h3 className="mb-4 border-b pb-3 text-lg font-semibold">Details</h3>
              ...
              {mode === "confirmDelete" && (
                <p className="text-gray-700">Are you sure you want to delete this record?</p>
              )}
              {extraContent && <div className="mt-4">{extraContent}</div>}
            </div>

            <div className="flex justify-end gap-3 px-6 py-4">
              {mode !== "viewProfile" && <button onClick={onClose} className="px-4 py-2 delete-btn">{cancelLabel || "Cancel"}</button>}
              {["add", "edit"].includes(mode) && <button onClick={handleSave} className="px-4 py-2 view-btn">{saveLabel || (mode === "edit" ? "Update" : "Save")}</button>}
              {mode === "confirmDelete" && <button onClick={handleDelete} className="px-4 py-2 edit-btn">{deleteLabel || "Yes, Delete"}</button>}
            </div>
          </div>
        </div>
