  ...
  return (
    <div className="bg-white shadow-lg rounded-2xl mt-6">
      <div className="flex justify-between items-center p-4 mb-6">
        ...
        <div ref={cartRef} onClick={() => navigate('/patientdashboard/cart')} className="relative cursor-pointer p-2 rounded-full border-2 border-[var(--primary-color)] bg-white shadow-sm hover:shadow-md transition group"><ShoppingCart size={26} className="text-[var(--primary-color)] group-hover:scale-110 transition-transform duration-200" />{cart.length > 0 && (<span className="absolute -top-1 -right-1 bg-red-500 text-white text-xs font-bold px-1.5 py-0.5 rounded-full border-2 border-white shadow-sm">{cart.length}</span>)}</div>
      </div>
            ...
            {tests.map(t => (
              <div key={t.id} className="card-stat">
                ...
                <div className="mt-3 flex gap-2">
                  <button onClick={() => navigate(`/patientdashboard/lab-tests/test/${t.id}`)} className="view-btn">View</button>
                  {renderButton(t, 't')}
